...
import { sendAlertInvestigationMessage, sendInvestigationMessage } from "@/lib/api";
import type {
  InvestigationEvidence,
  InvestigationMessage,
} from "@/lib/types";
import { toolLabel, toolSummary } from "@/lib/investigation";
...
export default function InvestigationPage() {
  ...
  const [messages, setMessages] = useState<InvestigationMessage[]>([
    {
      role: "assistant",
      content: "請從某筆資安事件點進來查證，或輸入明確的端點、來源 IP、時間範圍。一般事件會先由 LLM 翻成白話；這裡只在需要更多線索時才查已接事件來源，目前主要查證能力是 Wazuh MCP。",
    },
  ]);
  ...
  const contextualExamples = useMemo(() => {
    ...
    const agent = context.agentName || "這台端點";
    return [
      "這筆事件是不是需要立刻處理？",
      context.sourceIp ? `${context.sourceIp} 最近 7 天還有攻擊其他端點嗎？` : "",
  ...
  async function submit(question?: string) {
    ...
    try {
      const response = context.alertId
        ? await sendAlertInvestigationMessage({
            alertId: context.alertId,
            question: content,
            messages,
          })
        : await sendInvestigationMessage(nextMessages);
    ...
    <div className="flex h-full flex-col">
      <header className="flex items-center justify-between border-b border-border bg-card px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">證據查詢</h1>
            <p className="text-sm text-muted-foreground">需要更多線索時，才查已接資料來源</p>
          </div>
        </div>
            ...
            <div className="mx-auto flex max-w-4xl flex-col gap-4">
              <Card className="border-amber-200 bg-amber-50 text-amber-950 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-100">
                <CardContent className="p-4 text-sm leading-6">
                  事件會先由 LLM 翻成白話；這頁是按需證據查詢工具。送出問題後才會讀取已接來源的紀錄，目前主要查 Wazuh MCP；不會封鎖 IP、不會隔離端點，也不會修改任何系統設定。
                </CardContent>
              </Card>
              {context.alertId && (
                <Card className="border-primary/20 bg-primary/5">
                  <CardContent className="grid gap-3 p-4 text-sm sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      ...
                      <div className="mt-1 font-medium">{context.summary || "資安事件"}</div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">端點</div>
            ...
            <div className="mx-auto max-w-4xl space-y-3">
              ...
              <form onSubmit={onSubmit} className="flex items-end gap-2">
                <Textarea
                  ...
                  placeholder="例如：查這台端點最近 24 小時是否還有異常"
                  disabled={loading}
                />
